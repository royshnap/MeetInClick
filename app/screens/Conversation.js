...
const Conversation = ({ route, navigation }) => {
  ...
  if (conversationLoadingError || !activeConversation) {
    return (
      <View>
        <Text>
          Opps! there was an error loading conversation.{" "}
          <Link to={{ screen: "Conversations" }}>Back to conversation list</Link>
        </Text>
      </View>
    );
  }
    ...
    <View>
      ...
      <FlatList
        ...
        renderItem={({ item: message }) => (
          <MessageItem otherUser={otherUser} key={message.id} message={message} />
        )}
