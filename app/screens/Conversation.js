...
const Conversation = ({ route, navigation }) => {
  ...
  useEffect(() => {
    ...
    const total = activeConversation.messages
      .filter(message => message.sender === user.id)
      .reduce((acc, message) => acc + message.content.replace(/\s+/g, '').length, 0);
    setTotalCharacters(total);
  ...
  return (
    <View style={styles.container}>
      ...
      <FlatList
        ...
        renderItem={({ item: message }) => (
          <MessageItem otherUser={otherUser} currentUser={user} message={message} />
        )}
      ...
      {user && ( // Only render the input and send button if the user is defined
        <>
          <TextInput
            ...
            maxLength={MAX_CHARACTERS - totalCharacters + messageContent.replace(/\s+/g, '').length}
            style={styles.input}
          />
          <TouchableOpacity style={styles.sendButton} onPress={sendMessage}>
            <Text style={styles.sendButtonText}>Send</Text>
