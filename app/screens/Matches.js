...
const ConversationMatches = ({ route, navigation }) => {
  ...
  const { ageRange = [18, 120], genderPreference = 'Both' } =
    route.params || {};
  ...
  const filteredMatches = useMemo(() => {
    ...
    if (
      !user ||
      !user.mainCategory ||
      !user.conversationTopics /*|| !currentLocation*/
    ) {
      return [];
    }
    ...
    const matches = conversationTopicResults.filter((otherUser) => {
      ...
      const distance = calculateDistance(
        ...
        otherUser.currentLocation.coords
      );
      ...
      return (
        sameMainCategory &&
        commonTopics /*&& distance <= interestRadius*/ &&
        withinAgeRange &&
        genderMatch
      );
    });
  ...
  return (
    <ImageBackground source={backgroundImage} style={styles.background}>
      <View style={styles.container}>
        ...
        {filteredMatches.length > 0 && (
          <Text style={styles.matchesText}>{t('Matches')}:</Text>
        )}
        <FlatList
          ...
          >
            <View style={styles.modalContainer}>
              <View style={styles.modalContent}>
                ...
                <Text style={styles.modalText}>
                  Main Category: {selectedUser.mainCategory}
                </Text>
                <Text style={styles.modalText}>
                  Topics: {selectedUser.conversationTopics.join(', ')}
                </Text>
                <Button
                  title={t('Close')}
